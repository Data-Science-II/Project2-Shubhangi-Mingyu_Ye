...
REPORT
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    def stepRegression (cols: Set [Int], index_q: Int = index_rSqBar, first: Int = 1): (Int, PredictorMat2) =
    {
        var j_mx   = -1                                                  // best column, so far
        var mod_mx = null.asInstanceOf [PredictorMat2]                   // best model, so far
        var fit_mx = noDouble                                            // best fit, so far

        val (for_j, for_mod_j) = forwardSel (cols)                       // most predictive variable from forward selection
        val (eli_j, eli_mod_j) = backwardElim (cols, first)              // least predictive variable from backward elimination
        for_mod_j.train ().eval ()
        eli_mod_j.train ().eval ()                                       // train model, evaluate QoF
        val for_fit_j = for_mod_j.fitA(0).fit(index_q)
        val eli_fit_j = eli_mod_j.fitA(0).fit(index_q)                   // new fit
        val mod_j = if (eli_fit_j > for_fit_j) eli_mod_j
                    else                       for_mod_j
        val fit_j = if (eli_fit_j > for_fit_j) eli_fit_j
                    else                       for_fit_j
        val j     = if (eli_fit_j > for_fit_j) eli_j
                    else                       for_j
        if (fit_j > fit_mx) { j_mx = j; mod_mx = mod_j; fit_mx = fit_j}
        (j_mx, mod_mx)                                                   // return column and model to add or remove
    } // stepRegression
     ...
     *  @param cross    whether to include the cross-validation QoF measure
     */
    def stepRegressionAll (index_q: Int = index_rSqBar, first: Int = 1, cross: Boolean = true): (Set [Int], MatriD) =
    {
        val rSq  = new MatrixD (x.dim2 - 1, 3)                           // R^2, R^2 Bar, R^2 cv
        val ini_cols = Set (0)                                           // start with no variable in modelv
        val (j_1, mod_j_1) = forwardSel(ini_cols)
        val cols_1 = ini_cols + j_1
        val (j_2, mod_j_2) = forwardSel(cols_1)
        val cols = cols_1 + j_2

        breakable { for (l <- 3 until x.dim2 - 1) {
            val (j, mod_j) = stepRegression (cols, first)                // variable to add or remove
            if (j == -1) break
            if (cols contains j) cols -= j
            else                 cols += j                               // decides whether to add or remove variable
            val fit_j = mod_j.fitA(0).fit
            rSq(l)    = if (cross) Fit.qofVector (fit_j, mod_j.crossValidate ())   // use new model, mod_j, with cross
                        else       Fit.qofVector (fit_j, null)                     // use new model, mod_j, no cross
            if (DEBUG) {
                val k = cols.size - 1
                println (s"==> stepRegressionAll: add (#$k) variable $j, qof = ${fit_j(index_q)}")
            } // if
        }} // breakable for
